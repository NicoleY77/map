 ...
 */
KISSY.add("gallery/kcharts/1.0/linechart/index",function(S,Base,Template,Raphael,BaseChart,ColorLib,HtmlPaper,Legend,Theme,undefined,Tip){
	...
	S.extend(LineChart,BaseChart,{
		...
		animateLine:function(points,lineIndex,callback){
			var self = this,
				...
				duration = _cfg.anim.duration || 5,
				easing = _cfg.anim.easing || "easeIn",
				len = points.length,
				// 每段直线的宽度
				aver_len = total_len/(len - 1),
				tmpStocks = [],
				sub_path,
				point_index = 0,
				from = 0,
				to = 0,
				step = Math.round(total_len/40),
				itv,
				$line = paper.path(sub_path).attr(_attr);
			    tmpStocks[0] = self.drawStock(points[0]['x'],points[0]['y'],self.processAttr(_cfg.points.attr,color.getColor(lineIndex).DEFAULT));

				var show_len = (function(){
							var len = _cfg.series.length,
								tmpLen = len;
							for (var i = 0;i < len;i++){
				...
				itv = setInterval(function(){
					...
					to += step;
					//当前渲染点的索引
					point_index = Math.floor(to/aver_len);

					if(!tmpStocks[point_index] && points[point_index]){
						tmpStocks[point_index] = self.drawStock(points[point_index]['x'],points[point_index]['y'],self.processAttr(_cfg.points.attr,color.getColor(lineIndex).DEFAULT));
					}
					...
					if(to >= total_len) {
						...
						clearInterval(itv);
						if(self._finished.length == show_len){
							callback && callback();
						}
					}
			...
			if(_cfg.anim){
				...
				for(var i in self._points){
					...
					var path = self.getLinePath(self._points[i]),
						...
						pointsAttr = self.processAttr(self._cfg.points.attr,curColor.DEFAULT),
						hoverAttr = self.processAttr(self._cfg.points.hoverAttr,curColor.HOVER),
						line = _cfg.series[i]['isShow'] == false ? undefined : self.drawLine(path,{stroke:curColor.DEFAULT});
		...
		drawStock:function(x,y,attr,type){
			...
			return x && y && paper.circle(x,y,_attr["r"],attr).attr(_attr).attr(attr);
		...
		**/
		...
		lineChangeTo:function(currentLineIndex){
			...
			self._lines[currentLineIndex]['line'] = self.drawLine(self.getLinePath(self._points[currentLineIndex]),S.mix({stroke:self.color.getColor(currentLineIndex).DEFAULT},_cfg.line.hoverAttr));
		...
		**/
		...
		animateGridsAndLabels:function(){
			...
			for(var i in self._labelY){
					self._labelY[i][0] && self._labelY[i][0].remove();
					self._gridsY[i][0] && self._gridsY[i][0].remove();
			}
			self.drawGridsY();
			self.drawLabelsY();
		},
