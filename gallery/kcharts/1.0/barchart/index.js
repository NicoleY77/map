 ...
 */
KISSY.add('gallery/kcharts/1.0/barchart/index',function(S,Template,BaseChart,Color,HtmlPaper,Legend,Theme,undefined,Tip){
		...
		*/
		getBarsPos:function(){
			var self = this,
				...
				areaWidth =  isY ? (self._pointsY[1].y - self._pointsY[0].y):(self._pointsX[1].x - self._pointsX[0].x)  , //area总宽度
		...
		*/
		...
		tipHandler:function(barGroup,barIndex){
			var self = this,
				...
				isY = _cfg.zoomType == "y" ? true : false,
				$tip = tip.getInstance();
				tipData = self._points[barGroup][barIndex]["dataInfo"] || {},
				$bar = self._bars[barGroup]['bars'][barIndex],
				...
				for(var i in self._bars)if(i != barIndex){
					...
					for(var j in self._bars[i]['bars']){
						...
						if(self._barsPos[i]){
							...
							barPos && self._bars[i]['bars'][j].animate({"height":barPos.height,"width":barPos.width,"marginTop":barPos.y - ctn.y,marginLeft:barPos.x - ctn.x},0.4,"easeOut",function(){
							});
